...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorRes> handleIllegalArgumentException(IllegalArgumentException ex) {
        ErrorRes response = new ErrorRes(4000, ex.getMessage(), HttpStatus.BAD_REQUEST, LocalDateTime.now());
    ...
    public ResponseEntity<ErrorRes> handleGeneralException(Exception ex) {
        ErrorRes response = new ErrorRes(9999, ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR, LocalDateTime.now());
